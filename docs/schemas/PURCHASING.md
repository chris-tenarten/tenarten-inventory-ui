...
`purchase_orders` owns the optional primary `jobs.id` relationship; job number/name snapshots; vendor snapshots; Ship To and Authorized By snapshots; lifecycle; commercial totals; revision identity; dates; and attribution. `purchase_order_lines` owns stable shared line identity and canonical `chip`, `resin`, `pigment`, `filler`, or `other` classification. The legacy-named `chip_purchase_order_line_details` table stores structured material, optional size, Resin Color, Component Type, packaging, Chip moisture, quantity, price, requested date, and canonical Production job identity.
...
Migration `20260917_004_purchase_order_material_taxonomy.sql` expands new line classification to `pigment`, `filler`, and `other` without rewriting historical rows or snapshots. Only Chip requires Size and Bag ordering. Resin retains structured Resin fields and assist behavior; the additional types use common material, packaging, quantity, unit, price, and description fields. Pending Receivals preserve these classifications as `Pigment`, `Filler`, or `Other` rather than projecting them as Chip/Aggregate.
