...
import { isTenOpsSystemInboxUser, loadInboxRecipients, loadInboxUnreadCount, loadConversationSummaries, type ConversationSummary } from "@/modules/my-work/inbox";
import type { WorkCollaborator } from "@/modules/my-work/types";
...
export default function GlobalMessaging(){
  ...
  const[recentMessages,setRecentMessages]=useState<ConversationSummary[]>([]);
  ...
  const loadRecent=useCallback((force=false)=>{if(!currentUserId||recentRequestRef.current||(!force&&recentLoadedRef.current))return;recentRequestRef.current=true;setRecentLoading(true);setError("");void Promise.all([loadConversationSummaries(),recentLoadedRef.current?Promise.resolve(null):loadInboxRecipients()]).then(([messages,people])=>{setRecentMessages(messages);if(people)setCollaborators(people.filter(user=>user.userId!==currentUserId));recentLoadedRef.current=true;}).catch(caught=>setError(caught instanceof Error?caught.message:"Unable to load recent messages.")).finally(()=>{recentRequestRef.current=false;setRecentLoading(false);});},[currentUserId]);
  const handleUnreadChange=useCallback((count:number)=>{setUnreadCount(count);if(awarenessVisibleRef.current)loadRecent(true);},[loadRecent]);
  const prepareWorkspace=useCallback((onReady:()=>void)=>{setModalReady(false);setError("");void (collaborators.length?Promise.resolve(collaborators):loadInboxRecipients()).then((people)=>{setCollaborators(people.filter(user=>user.userId!==currentUserId));setModalReady(true);onReady();}).catch(caught=>setError(caught instanceof Error?caught.message:"Unable to load Messaging."));},[collaborators,currentUserId]);
  ...
  const conversations=useMemo(()=>recentMessages.map(item=>({...item,isSystem:isTenOpsSystemInboxUser(item.userId)})),[recentMessages]);
  ...
  useEffect(()=>{if(!currentUserId)return;let timer:ReturnType<typeof setTimeout>|undefined;const flush=()=>{window.dispatchEvent(new Event("tenops:messaging-changed"));refreshUnread(currentUserId);if(awarenessVisibleRef.current)loadRecent(true);};const refresh=()=>{clearTimeout(timer);timer=setTimeout(flush,100);};const channel=supabase.channel(`global-messaging-badge:${currentUserId}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"my_work_messages",filter:`recipient_user_id=eq.${currentUserId}`},refresh).on("postgres_changes",{event:"UPDATE",schema:"public",table:"my_work_messages",filter:`recipient_user_id=eq.${currentUserId}`},refresh).subscribe();return()=>{clearTimeout(timer);void supabase.removeChannel(channel);};},[currentUserId,loadRecent,refreshUnread]);
  ...
  return <>
    ...
    {modalReady?<MessagingErrorBoundary key={sidebarExpanded?"sidepane":modalOpen?"modal":"closed"} onClose={()=>{setModalOpen(false);setSidebarExpanded(false);}}><InboxDialog open={modalOpen||sidebarExpanded} onClose={()=>{if(sidebarExpanded)setSidebarExpanded(false);else setModalOpen(false);}} currentUserId={currentUserId} collaborators={collaborators} initialUserId={initialUserId} onUnreadChange={handleUnreadChange} presentation={sidebarExpanded?"sidepane":"dialog"}/></MessagingErrorBoundary>:null}
  </>;
}
