...
export const SAMPLE_FORMULATION_CALCULATION_VERSION='sample-formulation-v2-mass-balance';
...
export function calculateSampleFormulation(state:SampleFormulationState,rows:FormulationRowInput[]){
  ...
  const isMassBalance=state.calculationVersion===SAMPLE_FORMULATION_CALCULATION_VERSION;
  ...
  const resinEffective=resinIndex<0?null:toOunces(rows[resinIndex].quantity,rows[resinIndex].unit);
