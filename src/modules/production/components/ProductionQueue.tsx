...
export default function ProductionQueue({ jobs, selectedJobId, attachmentCounts, onSelectJob }: Props) {
  return <div className="overflow-hidden border border-slate-400 bg-white shadow-sm"><div className="hidden grid-cols-[minmax(220px,1.7fr)_125px_160px_140px_115px_145px] gap-2 border-b border-slate-400 bg-slate-100 px-4 py-2 text-xs font-bold uppercase tracking-wide text-slate-600 md:grid"><span>Job</span><span>Status</span><span>Schedule</span><span>Delivery</span><span>Material</span><span>Planning</span></div><div className="divide-y divide-slate-300">
    {jobs.map((job) => {
      ...
      return <div key={job.id} className={`relative grid grid-cols-1 items-center gap-2 px-4 py-3 transition hover:bg-blue-50 md:grid-cols-[minmax(220px,1.7fr)_125px_160px_140px_115px_145px] ${selectedJobId === job.id ? 'bg-blue-50 ring-2 ring-inset ring-blue-600' : ''}`}>
