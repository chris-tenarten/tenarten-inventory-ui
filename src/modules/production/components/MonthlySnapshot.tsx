...
function DailyLaborChart({ values }: { values: Array<{ date: string; hours: number }> }) {
  ...
  const ceiling = maximum <= 10 ? Math.max(1, Math.ceil(maximum)) : Math.ceil(maximum / 10) * 10;
  const yTicks = [0, 0.25, 0.5, 0.75, 1];
  const xTicks = [0, 7, 14, 21, values.length - 1];
  const reportedDays = values.filter((item) => item.hours > 0);
  const shortDate = (value: string) => new Date(`${value}T12:00:00`).toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
  const left = 46; const top = 10; const width = 578; const height = 116; const barStep = width / values.length; const barWidth = Math.max(3, barStep - 3);
  return <div className="mt-4 border-y border-slate-200 py-3"><div className="flex items-baseline justify-between gap-3"><h3 className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">Daily Reported Labor</h3><span className="text-[10px] text-slate-500">Peak {hours(maximum)}</span></div>{maximum > 0 ? <div className="mt-3 grid gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(180px,1fr)]"><div className="min-w-0"><svg viewBox="0 0 640 158" role="img" aria-labelledby="daily-labor-chart-title daily-labor-chart-description" className="block h-auto w-full"><title id="daily-labor-chart-title">Daily reported labor hours</title><desc id="daily-labor-chart-description">Bar chart of labor hours reported on each day during the last 30 days. Exact values are also listed beside the chart.</desc>{yTicks.map((tick) => { const y = top + height - tick * height; return <g key={tick}><line x1={left} x2={left + width} y1={y} y2={y} stroke="#cbd5e1" strokeWidth="0.7" /><line x1={left - 4} x2={left} y1={y} y2={y} stroke="#64748b" strokeWidth="1" /><text x={left - 7} y={y + 3} textAnchor="end" fontSize="9" fill="#64748b">{new Intl.NumberFormat(undefined, { maximumFractionDigits: 1 }).format(ceiling * tick)}h</text></g>; })}{values.map((item, index) => { const barHeight = item.hours / ceiling * height; return <rect key={item.date} x={left + index * barStep + (barStep - barWidth) / 2} y={top + height - barHeight} width={barWidth} height={Math.max(item.hours > 0 ? 1.5 : 0, barHeight)} fill="#1d4ed8"><title>{`${formatDate(item.date)}: ${hours(item.hours)}`}</title></rect>; })}<line x1={left} x2={left + width} y1={top + height} y2={top + height} stroke="#475569" strokeWidth="1" />{xTicks.map((index) => { const x = left + index * barStep + barStep / 2; return <g key={index}><line x1={x} x2={x} y1={top + height} y2={top + height + 4} stroke="#64748b" strokeWidth="1" /><text x={x} y={top + height + 16} textAnchor={index === 0 ? 'start' : index === values.length - 1 ? 'end' : 'middle'} fontSize="9" fill="#64748b">{shortDate(values[index].date)}</text></g>; })}</svg></div><section className="min-w-0"><h4 className="text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500">Reported Days</h4><div className="mt-1 max-h-40 overflow-y-auto border-y border-slate-200">{reportedDays.map((item) => <div key={item.date} className="flex min-h-8 items-center justify-between gap-3 border-b border-slate-100 px-1 text-xs last:border-b-0"><span className="text-slate-600">{shortDate(item.date)}</span><strong className="tabular-nums text-slate-900">{hours(item.hours)}</strong></div>)}</div></section></div> : <p className="mt-3 text-xs text-slate-500">No manpower was reported during this period.</p>}</div>;
}
