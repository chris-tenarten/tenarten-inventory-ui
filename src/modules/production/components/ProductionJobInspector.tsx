...
function activityDescription(change: ProductionJobActivity) {
  ...
  if (change.event_type === 'attachments_uploaded') {
    const names = Array.isArray(metadata.file_names) ? metadata.file_names.filter((name): name is string => typeof name === 'string') : [];
    return { action: names.length === 1 ? 'Added an attachment' : `Added ${names.length || 'multiple'} attachments`, detail: names.join(', ') || undefined };
  }
...
export default function ProductionJobInspector({ job, onClose, onUpdateJob, onStageSchedule, onAttachmentsChanged, initialFocus }: Props) {
  ...
  useEffect(() => {
    ...
    loadProductionJobActivity(job.id)
      .then((changes) => { if (live) setActivity(changes); })
      .catch((loadError: unknown) => { if (live) setActivityError(loadError instanceof Error ? loadError.message : 'Unable to load recent changes.'); })
  ...
  useEffect(() => {
    ...
    loadJobAttachments(job.id)
      .then((files) => { if (live) setAttachments(files); })
      .catch((loadError: unknown) => { if (live) setAttachmentError(loadError instanceof Error ? loadError.message : 'Unable to load attachments.'); })
      .finally(() => { if (live) setAttachmentsLoading(false); });
    requestAnimationFrame(() => {
      const target = initialFocus && initialFocus !== 'attachments' ? panel.current?.querySelector<HTMLElement>(`[data-field="${initialFocus}"]`) : null;
      if (target) { target.focus(); } else closeRef.current?.focus();
    });
    return () => { live = false; };
  }, [initialFocus, job.id]);
  ...
  async function openAttachment(attachment: JobAttachment) {
    ...
    catch (error) { setAttachmentError(error instanceof Error ? error.message : 'Unable to open attachment.'); }
  }
  ...
  const tabs: Array<{ id: InspectorSection; label: string }> = [
    ...
    { id: 'files', label: `Files${attachments.length ? ` (${attachments.length})` : ''}` },
