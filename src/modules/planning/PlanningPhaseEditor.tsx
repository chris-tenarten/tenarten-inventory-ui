    ...
    <div className="fixed inset-0 z-[140] flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={selectedItem ? "planning-item-title" : "planning-phase-title"} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-slate-400 bg-white p-5 shadow-2xl">
        ...
        {phase && draft.timeline !== "pause" && <div className="mt-4 border border-slate-300 bg-slate-50 p-3">
          <div className="flex items-end justify-between gap-3"><div><div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">Planning Progress</div><div className="mt-1 text-2xl font-bold text-slate-950">{progress.percent}%</div></div><div className="text-right text-xs text-slate-600"><div>{formatPlanningHours(progress.completedHours)} / {formatPlanningHours(progress.totalHours)} planned hrs</div><div className="mt-1">{progress.completedItems} / {progress.totalItems} Items complete</div></div></div>
