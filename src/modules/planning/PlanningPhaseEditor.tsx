...
import type { PlanningPhase, PlanningPhaseInput, PlanningStatus, TimelineBehavior } from "./types";
...
export default function PlanningPhaseEditor({ job, phase, phases, libraryEntry, phaseLimitReached, onSave, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="planning-phase-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border border-slate-400 bg-white p-5 shadow-2xl">
