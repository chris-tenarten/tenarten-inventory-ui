...
export default function PlanningItemEditor({ phaseId, item, sortOrder, onSave, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/45 p-4" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="planning-item-title" className="w-full max-w-lg border border-slate-400 bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <h2 id="planning-item-title" className="text-lg font-bold">{item ? "Edit Item" : "Add Item"}</h2>
          <button type="button" onClick={requestClose} className="h-9 border border-slate-300 px-3 text-sm font-bold">Close</button>
        </div>
