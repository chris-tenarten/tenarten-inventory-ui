...
export async function loadMyWorkTasks():Promise<WorkTask[]>{
  const[{data,error},attachments]=await Promise.all([supabase.rpc('list_my_work_tasks'),supabase.from('work_task_attachments').select('task_id')]);if(error)throw error;if(attachments.error)throw attachments.error;const counts=new Map<string,number>();for(const attachment of attachments.data??[])counts.set(attachment.task_id,(counts.get(attachment.task_id)??0)+1);return((data??[]) as TaskRow[]).map(row=>({id:row.id,title:row.title,notes:row.notes||'',visibility:row.visibility,creatorUserId:row.creator_user_id,creatorName:row.creator_name||'TenOps user',assigneeUserId:row.assignee_user_id,assigneeName:row.assignee_name||'TenOps user',dueDate:row.due_date||'',contextType:row.context_type||'',contextId:row.context_id||'',jobNumber:row.job_number||'',jobName:row.job_name||'',jobCustomer:row.job_customer||'',color:row.color_key||'neutral',attachmentCount:counts.get(row.id)??0,completedAt:row.completed_at||'',createdAt:row.created_at,updatedAt:row.updated_at}));
}
...
export async function uploadWorkTaskAttachments(taskId:string,files:File[]):Promise<WorkTaskAttachment[]>{
  ...
  for(const file of files){
    ...
    const stored=await supabase.storage.from(ATTACHMENT_BUCKET).upload(storagePath,file,{contentType,upsert:false});if(stored.error)throw stored.error;
