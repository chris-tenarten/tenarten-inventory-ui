'use client';
import CuratedDemoBadge from './CuratedDemoBadge';
...
import { planningIntervalGeometry, rangesIntersect } from '@/modules/planning/timeline-model.mjs';
import { overlayVisualForPhase, PLANNING_PAUSE_HATCH } from '@/modules/planning/phase-visuals';
import type { Bid } from './types';
import { adjustProjectedWindow, type BidPlanning, type WindowEdit } from './planning';
import { saveBidProjectedWindow } from './planning-data';

import type { PlanningMode, PlanningRecord } from './planning-model';
...
export default function IntakePlanningTimeline({ bids, onSelectBid, onChanged, mode, setMode, items, loaded, productionLoaded, onRefresh }: {
  bids: Bid[]; onSelectBid(bid: Bid): void; onChanged(): Promise<unknown>;
  mode: PlanningMode; setMode(mode: PlanningMode): void; items: PlanningRecord[];
  loaded: boolean; productionLoaded: boolean;
  ...
  function fit() {
    ...
    for (const item of items) {
      if (item.start && item.end) ranges.push({start:item.start,end:item.end});
      if (item.source === 'production') for (const phase of item.phases) {
        if (phase.start_date && phase.end_date) ranges.push({start:phase.start_date,end:phase.end_date});
      }
    }
  ...
  function begin(event: PointerEvent<HTMLDivElement>, record: BidPlanning) {
    if (!canEdit() || pendingId || event.button !== 0 || !record.projected_production_start || !record.projected_production_end) return;
  ...
  return <section className="mt-4 min-w-0" aria-label="Intake Planning">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-lg font-bold">Projected Production Planning</h2><p className="mt-1 text-xs text-slate-600">Dashed PROJECTED = potential Intake work. Solid PRODUCTION = committed work. Projections do not reserve capacity.</p></div><div className="flex flex-wrap items-center gap-3"><button type="button" disabled={Boolean(pendingId)} onClick={() => { setError(''); void onRefresh().catch(caught => setError(caught.message)); }} className="min-h-10 border border-slate-300 px-3 text-sm">Refresh planning</button></div></div>
    <div ref={controls} data-planning-controls className="my-3 flex flex-wrap items-center gap-3"><div role="group" aria-label="Planning sources" className="flex flex-wrap border border-slate-300">{(['intake', 'production', 'combined'] as const).map(value => <button key={value} type="button" aria-pressed={mode === value} onClick={() => setMode(value)} className={`min-h-10 px-3 text-xs font-bold ${mode === value ? 'tenops-selected-surface' : 'bg-white text-slate-600'}`}>{value === 'intake' ? 'Projected Intake' : value === 'production' ? 'Production' : 'Combined'}</button>)}</div><div role="group" aria-label="Timeline viewport" className="flex flex-wrap items-center gap-2"><span className="text-xs font-semibold text-slate-600">Zoom</span><div className="flex items-center gap-1"><button type="button" aria-label="Zoom out" title="Zoom out" disabled={dayWidth <= MIN_DAY_WIDTH} onClick={() => zoomBy(1 / 1.5)} className={`${viewportButton} min-w-10 text-base`}>−</button><button type="button" aria-label="Zoom in" title="Zoom in" disabled={dayWidth >= MAX_DAY_WIDTH} onClick={() => zoomBy(1.5)} className={`${viewportButton} min-w-10 text-base`}>+</button></div><button type="button" onClick={goToToday} title="Show today with upcoming dates ahead" className={viewportButton}>Today</button><button type="button" onClick={fit} disabled={!loaded || (mode !== 'intake' && !productionLoaded)} className={viewportButton}>Fit</button></div></div>
    ...
    <div ref={viewport} onPointerDown={beginPan} onPointerMove={event => { if (pan.current) event.currentTarget.scrollLeft = pan.current.scroll - (event.clientX - pan.current.x); }} onPointerUp={endPan} onPointerCancel={endPan} onLostPointerCapture={endPan} role="region" aria-label="Scrollable planning timeline" tabIndex={0} className="max-h-[65dvh] min-w-0 overflow-auto border border-slate-300 bg-white" onKeyDown={event => { if (event.key === 'Escape') { cancel(); endPan(); } }}>
      <div style={{ minWidth: width + RAIL_WIDTH }}>
        ...
        <div className="relative" data-timeline-body>
          {todayVisible && <span aria-hidden="true" data-today-line data-continuous-today className={`pointer-events-none absolute inset-y-0 z-[5] border-l ${styles.todayLine}`} style={{ left: RAIL_WIDTH + todayLeft }} />}
        {items.map(item => {
          if (item.source === 'intake') { const bid = item.bid;
          const canWrite = canEdit();
          const record = item.planning; const current = drag?.record.id === bid.id ? drag : null;
          ...
          return <div key={bid.id} data-intake-planning-row={bid.id} className="flex min-h-14 border-b border-slate-200"><button type="button" onClick={() => onSelectBid(bid)} title={bid.projectName} className={railClass}><span className="flex min-w-0 items-center gap-1"><strong className="truncate text-xs">{bid.projectName}</strong><CuratedDemoBadge bidId={bid.id}/></span><span className="text-[10px] text-slate-500">INTAKE · {bid.status === 'won' ? 'Won' : 'Active'}</span></button><div data-planning-canvas className="relative h-14 shrink-0 cursor-grab" style={canvasStyle}>
            {visible ? <div data-projected-block onPointerDown={canWrite ? event => begin(event, record) : undefined} onPointerMove={canWrite ? move : undefined} onPointerUp={canWrite ? () => void finish() : undefined} onPointerCancel={canWrite ? cancel : undefined} className="absolute top-2 flex h-10 touch-none items-center overflow-hidden rounded border border-dashed border-blue-600 hover:border-blue-800 focus-within:ring-2 focus-within:ring-blue-600 bg-blue-100/60 text-blue-950" style={{ ...geometry(start!, end!), backgroundImage: 'repeating-linear-gradient(135deg, transparent 0 8px, rgba(59,130,246,.08) 8px 12px)' }} title={`PROJECTED: ${start} – ${end}. Tentative; no capacity reservation.`}>
          ...
          } else { const job = item.job;
          const jobPhases = item.phases;
          return <div key={item.key} data-production-planning-row={job.id} className="flex border-b border-slate-200"><button type="button" onClick={() => openProductionJob(job.id)} title={job.name} className={railClass}><strong className="block truncate text-xs">{job.job_number ? `${job.job_number} · ` : ''}{job.name}</strong><span className="text-[10px] text-slate-500">PRODUCTION{job.rework_cycle ? ' · REWORK' : ''}</span></button><div data-planning-canvas className="relative shrink-0 cursor-grab" style={{ ...canvasStyle, height: 64 + jobPhases.length * 24 }}>
    ...
    <p ref={hint} className="mt-2 text-xs text-slate-500">{canWrite ? 'Drag blank space to pan · Drag projected blocks to move · Drag edges to resize · Open a row to edit dates with the keyboard.' : 'Drag blank space to pan · Open a row to view details · Intake is read-only for your role.'}</p>
  </section>;
}
