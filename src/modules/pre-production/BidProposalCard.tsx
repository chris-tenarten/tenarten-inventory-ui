...
export default function BidProposalCard({bidId}:{bidId:string}){
 const auth=useAuth();const canWrite=Boolean(auth.profile?.isActive&&auth.can('accessIntake'));
 ...
 const description=error?'Proposal relationship unavailable.':allowed===false?'Proposal access is required.':allowed===null?'Loading Proposal relationship…':latest?`${latest.estimateNumber} · ${latest.status==='issued'?'Issued':'Draft'}${proposals.length>1?` · ${proposals.length} linked`:''}`:canWrite?'Create or manage this Bid’s commercial proposal.':'No linked Proposal.';
 return <button type="button" disabled={allowed===false||(!canWrite&&!latest)} onClick={()=>window.location.assign(`/proposals?bid=${encodeURIComponent(bidId)}${latest?`&open=${encodeURIComponent(latest.id)}`:''}`)} className="group min-h-24 w-full rounded-sm border border-slate-200 bg-slate-50 p-3 text-left transition hover:border-slate-400 hover:bg-white hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-slate-500 transition group-hover:text-blue-800"/><h3 className="text-xs font-bold uppercase tracking-[0.05em] text-slate-800">Proposal &amp; Estimate</h3></div><p className={`mt-2 text-xs leading-5 ${error?'font-semibold text-red-700':'text-slate-600'}`}>{description}</p></button>;
}
