...
function EditableEntryRow({ entry, jobs, workers, tasks, addWorker, addTask, onSaved, selected, onSelected }: {
  ...
  useEffect(() => setDraft(entryDraft(entry)), [entry]);
  useEffect(() => {
    ...
    <div className="mx-auto w-full max-w-[1800px] px-3 py-5 sm:px-5 sm:py-7">
      <div className="flex flex-col gap-3 border-b border-slate-400 pb-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => void load()} disabled={loading} className="inline-flex h-9 items-center gap-2 border border-slate-400 bg-white px-3 text-xs font-bold uppercase tracking-wide"><RotateCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Refresh</button>
        </div>
      </div>

      {manageReferences && <div className="mt-4 grid gap-4 border border-slate-400 bg-white p-4 lg:grid-cols-2">
        <div className="flex items-center justify-between gap-3 border-b border-slate-300 pb-3 lg:col-span-2"><div><h2 className="text-sm font-bold text-slate-900">Workers & Tasks</h2><p className="text-xs text-slate-600">Maintain names, display order, and active status.</p></div><button type="button" onClick={closeReferencePanel} className="h-9 border border-slate-500 bg-white px-3 text-xs font-bold uppercase tracking-wide text-slate-800">Close</button></div>
