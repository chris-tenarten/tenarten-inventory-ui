...
async function fixture(page: Page, role = 'lead') {
  ...
  const job = { id:'job',name:'Test Job',job_number:'26-001',production_status:'in_production',archived_at:null };
...
test('Job totals, pivots, inactive history, filters and safe group identity', async ({page})=>{
  ...
  await expect(page.locator('summary').filter({hasText:'Uncategorized'})).toContainText('2.00 h');
  ...
  await expect(page.locator('summary')).toContainText('Rough Grind on Polisher');
