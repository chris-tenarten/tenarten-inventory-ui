...
-- Fail before any DDL if the live legacy schema does not match the receipt
-- contract used by the application. The entire migration is transactional.
do $preflight$
declare
...
begin
  if to_regclass('public.jobs') is null
    ...
    raise exception 'Migration 20260714_005 requires jobs, pending_receivals, inventory_items, and inventory_transactions.';
  end if;

  if (select atttypid from pg_attribute where attrelid = 'public.jobs'::regclass and attname = 'id' and not attisdropped) <> 'uuid'::regtype
    or (select atttypid from pg_attribute where attrelid = 'public.pending_receivals'::regclass and attname = 'id' and not attisdropped) <> 'uuid'::regtype then
    raise exception 'Migration 20260714_005 requires uuid jobs.id and pending_receivals.id.';
  end if;
  ...
  if missing_columns is not null then
    raise exception 'Migration 20260714_005 is incompatible; missing columns: %', missing_columns;
  end if;

  if to_regprocedure('public.receive_pending_receival(uuid,text)') is null then
    raise exception 'Expected legacy function public.receive_pending_receival(uuid,text) was not found. Run the preflight inspection script.';
  end if;

  if to_regprocedure('public.receive_pending_receival_with_reservation(uuid,text)') is not null then
    raise exception 'receive_pending_receival_with_reservation(uuid,text) already exists; migration may already be applied.';
  end if;
end;
$preflight$;
...
alter table public.pending_receivals
  ...
  add column temporary_job_label text;
...
comment on column public.pending_receivals.production_job_id is
  'Canonical Production job reservation. Mutually exclusive with temporary_job_label.';
comment on column public.pending_receivals.temporary_job_label is
  'Unlinked reservation identity retained when no canonical Production job exists.';
comment on column public.inventory_items.production_job_id is
  'Canonical Production job owning this reserved inventory balance.';
comment on column public.inventory_items.temporary_job_label is
  'Unlinked reservation identity for this inventory balance.';
comment on column public.inventory_transactions.production_job_id is
  'Canonical Production job reservation captured with this inventory event.';
comment on column public.inventory_transactions.temporary_job_label is
  'Unlinked reservation label captured with this inventory event.';

-- Only exact, unique normalized job-number matches are safe to link automatically.
with unique_job_numbers as (
  select lower(trim(job_number)) as normalized_job_number, min(id::text)::uuid as job_id
  from public.jobs
  where nullif(trim(job_number), '') is not null
  group by lower(trim(job_number))
  having count(*) = 1
)
update public.pending_receivals receival
set production_job_id = match.job_id
from unique_job_numbers match
where receival.is_earmarked is true
  and lower(trim(receival.earmarked_job_name)) = match.normalized_job_number;

update public.pending_receivals
set temporary_job_label = nullif(trim(earmarked_job_name), '')
where is_earmarked is true
  and production_job_id is null
  and nullif(trim(earmarked_job_name), '') is not null;
...
set production_job_id = match.job_id
from unique_job_numbers match
where inventory.earmarked_for_job is true
  and lower(trim(inventory.earmarked_job)) = match.normalized_job_number;

update public.inventory_items
set temporary_job_label = nullif(trim(earmarked_job), '')
where earmarked_for_job is true
  and production_job_id is null
  and nullif(trim(earmarked_job), '') is not null;
...
update public.inventory_transactions history
set production_job_id = match.job_id
from unique_job_numbers match
where history.is_earmarked is true
  and lower(trim(history.earmarked_job_name)) = match.normalized_job_number;
...
create index pending_receivals_production_job_idx
  on public.pending_receivals (production_job_id) where production_job_id is not null;
create index inventory_items_production_job_idx
  on public.inventory_items (production_job_id) where production_job_id is not null;
create index inventory_transactions_production_job_idx
  on public.inventory_transactions (production_job_id) where production_job_id is not null;
...
begin
  ...
  -- Preserve the legacy receipt contract: every reserved receival becomes its
  -- own lot, while only unreserved stock may merge into an existing row.
