...
begin
  ...
  then
    raise exception 'JOB_TRANSMITTAL_EDITABLE_CUSTOMER_PREDECESSOR_MISSING';
  end if;
end $$;

-- Keep the canonical job UUID relationship while preserving the operator's
-- document-specific Customer Name in the immutable issued snapshot.
...
comment on function public.issue_job_transmittal(uuid,text,jsonb,text) is
  'Issues an immutable job-linked Transmittal while preserving document-specific display fields, including editable Customer Name.';
