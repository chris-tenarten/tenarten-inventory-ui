...
export function buildPurchaseOrderPdfModel(orderSnapshot, linesSnapshot) {
  ...
  const lines = [...linesSnapshot]
    ...
    .map((line) => ({
      ...
      description: text(line.display_description) || [
        text(line.material),
        text(line.chip_size),
        [text(line.package_quantity), text(line.package_measure), text(line.container_type)].filter(Boolean).join(" "),
        text(line.moisture_condition),
      ].filter(Boolean).join(", "),
  ...
  return {
    ...
    job: linked ? {
      ...
      customer: text(orderSnapshot.customer),
    } : {
      kind: "stock",
      number: "",
      name: "Stock Purchase",
