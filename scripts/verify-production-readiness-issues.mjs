...
assert.match(table, /function focusMissingScheduleField/);
assert.match(table, /const field = !row\.plannedStart \? 'plannedStart' : 'plannedEnd'/);
assert.match(table, /input\?\.scrollIntoView\(\{ block: 'nearest', inline: 'nearest' \}\)/);
assert.match(table, /input\?\.focus\(\)/);
assert.match(table, /focusMissingScheduleField\(job, row\)/);
assert.doesNotMatch(table, /onScheduleJob/);
