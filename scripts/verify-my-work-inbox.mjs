...
assert.doesNotMatch(dialog, /reaction|typing indicator|delivered|read receipt/i);
...
assert.match(page, /Promise\.all\(\[loadMyWorkTasks\(\), loadWorkCollaborators\(\), loadWorkJobs\(\)\]\)/, "Inbox initialization must not share the task-loading failure boundary");
assert.doesNotMatch(page, /Promise\.all\(\[[^\]]*loadInboxRecipients/, "Inbox-only RPC failure must not produce the My Work task error");
assert.match(page, /loadInboxRecipients\(\)\.then\(setInboxRecipients\)\.catch\(\(\)=>setInboxRecipients\(\[\]\)\)/);
