...
assert.match(workspace, /onCreated=\{setCreatedJob\}/);
assert.match(workspace, /onScheduleJob=\{openJobScheduling\}/);
assert.match(queue, /<UnscheduledBadge compact onClick=\{\(\) => onScheduleJob\(job\)\}/);
assert.match(table, /<UnscheduledBadge compact onClick=\{\(\) => onScheduleJob\(job\)\}/);
