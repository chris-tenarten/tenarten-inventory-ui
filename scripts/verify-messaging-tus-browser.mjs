 ...
 * Streams 4 x 50 MB in 6 MiB chunks; never contacts Supabase. Not a hosted Storage test.
 */
...
const server=createServer(async(req,res)=>{
 ...
 if(path==='/download'){
  ...
  res.setHeader('Content-Length','50000000');let sent=0;const chunk=Buffer.alloc(64*1024);
  const pump=()=>{while(sent<50000000){const part=chunk.subarray(0,Math.min(chunk.length,50000000-sent));sent+=part.length;downloaded+=part.length;if(!res.write(part)){res.once('drain',pump);return;}}res.end();};pump();return;
 }
 if(req.headers.authorization!=='Bearer local-test-token'){res.writeHead(401).end();return;}
 if(req.method==='POST'){
  const id=String(++next);const size=Number(req.headers['upload-length']);assert(size<=50000000);
...
try{
 ...
 const result=await page.evaluate(async(base)=>{
  const part=new Uint8Array(1_000_000);const files=[new File(Array(50).fill(part),'drawing.dwg'),new File(Array(50).fill(part),'installer.exe'),new File(Array(50).fill(part),'archive.unknown'),new File(Array(50).fill(part),'model.bin')];
 ...
 assert.equal(result.progress,50000000);assert(result.retries>0);assert(heads>0);assert.equal([...uploads.values()].reduce((sum,item)=>sum+item.offset,0),200000000);
 ...
 const download=await downloadEvent;assert.equal(download.suggestedFilename(),'drawing & #.dwg');await download.saveAs(`${dir}/download.dwg`);assert.equal(downloaded,50000000);
 ...
 const canceled=await page.evaluate(async(base)=>{
  ...
  try{await window.messaging.uploadResumable({baseUrl:base,id:crypto.randomUUID(),entry:{id:crypto.randomUUID(),name:'cancel.zip',size:50000000,contentType:'application/octet-stream'},file:new File(Array(50).fill(new Uint8Array(1000000)),'cancel.zip'),signal:controller.signal,credentials:async()=>({token:'local-test-token',key:'local'}),progress:(bytes)=>{if(bytes>0)controller.abort();}});}catch{stopped=true;}
  return stopped;
 },base);assert(canceled);
 await writeFile(`${dir}/tus-results.json`,JSON.stringify({bytesUploaded:200000000,bytesDownloaded:downloaded,maxChunk,patches,heads,retryEvents:result.retries,canceled,scope:'Real Chromium + actual TUS client; local protocol fixture, not Supabase Storage'},null,2));
 console.log('PASS: browser TUS 4 x 50 MB, bounded chunks, 503 retry/HEAD resume, progress, active abort, streamed 50 MB native download.');
