...
assert(clientShell.includes('auth.ready') && clientShell.includes('Restoring your TenOps session…'), "Login credentials must remain hidden while the persisted session is restoring");
assert(clientShell.includes('<AccountAccessPanel onAuthenticated={() => {}} separated={false} />'), "Locked shell must expose normal account authentication after session restoration completes");
...
assert(!accountAccess.includes('CUTOVER_NOTICE_KEY') && !accountAccess.includes('TenOps account sign-in is changing'), "The obsolete RBAC/login cutover announcement must not remain active");
...
assert(supabaseClient.includes('persistSession: true'), "Supabase browser sessions must persist across visits");
assert(supabaseClient.includes('autoRefreshToken: true'), "Persisted Supabase sessions must refresh normally");
assert(supabaseClient.includes('detectSessionInUrl: true'), "Setup and Recovery callback sessions must still be detected");
assert(authProvider.includes('supabase.auth.getSession()'), "Auth startup must restore the persisted Supabase session before becoming ready");
