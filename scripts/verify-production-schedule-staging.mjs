...
assert.equal(workspaceSource.includes('onSaveSchedule={openApprovalDialog}'), false);
assert.equal(workspaceSource.includes('if (auth.isAuthenticated && auth.profile?.isActive)'), true);
assert.equal(workspaceSource.includes('changedByName: auth.profile.displayName'), true);
assert.equal(workspaceSource.includes('if (!(auth.isAuthenticated && auth.profile?.isActive)) {'), true);
...
assert.equal(inspectorSource.includes('onClick={onSaveSchedule}'), false);
assert.equal(inspectorSource.includes('Save job details first. Planned dates will remain staged.'), true);
assert.equal(inspectorSource.includes('Schedule incomplete. Enter both planned dates to stage this change for Save All.'), true);
