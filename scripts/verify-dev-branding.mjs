...
const [shell, appBranding, overlays, layout, appearance, styles] = await Promise.all([
...
assert.match(shell, /HeaderProductName loginGate=\{!isUnlocked\}/);
assert.match(shell, /HeaderBrandArtwork loginGate=\{!isUnlocked\}/);
assert.match(shell, /HeaderEnvironmentIdentity loginGate=\{!isUnlocked\}/);
...
assert.match(overlays, /w-\[5\.1rem\][\s\S]*sm:w-\[5\.75rem\]/);
assert.match(overlays, /w-12[\s\S]*sm:w-\[3\.6rem\]/);
