...
const [helper, badge, shell, bulletin, transmittal, planning, manifest] = await Promise.all([
  read("src/lib/early-access.mjs"),
  read("src/components/EarlyAccessBadge.tsx"),
  read("src/app/client-layout-shell.tsx"),
  read("src/modules/production/components/WhatsNewBulletin.tsx"),
...
assert.match(bulletin, /EARLY_ACCESS_ENABLED[\s\S]*planningAnnouncement[\s\S]*letterOfTransmittalAnnouncement/);
assert.match(bulletin, /Customer Name is now editable/);
assert.match(bulletin, /Line breaks are preserved in Preview and generated PDFs/);
assert.match(bulletin, /title: "Planning"/);
assert.match(bulletin, /Settings → Phase Library/);
assert.doesNotMatch(bulletin, /title: "Job Updates"/);
assert.doesNotMatch(bulletin, /Letter of Transmittal Early Access|Planning Early Access/);
