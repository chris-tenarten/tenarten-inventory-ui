...
assert.equal(result.finishedAreaSf,'1');assert.equal(result.areaSf,'1');assert.equal(result.productionVolumeCft,'0.0313');assert.equal(result.calculatedWeightPerSf,'4');assert.equal(result.effectiveWeightPerSf,'4');assert.equal(result.totalFormulaWeightOz,'100');assert.equal(result.nonChipWeightOz,'36');assert.equal(result.availableChipMixOz,'64');
...
result=calculateSampleFormulation({...standard,resinParts:'4',ratioProvenance:'manual'},rows);assert.equal(result.rows[7].effectiveQuantity,'3.75');assert.equal(result.availableChipMixOz,'63.25');
result=calculateSampleFormulation(standard,rows.map((row,index)=>index===5?{...row,quantity:'36'}:row));assert.equal(result.availableChipMixOz,'46');assert.deepEqual(result.rows.slice(0,5).map(row=>row.effectiveQuantity),['18.4','13.8','9.2','2.3','2.3']);
result=calculateSampleFormulation(standard,rows.map((row,index)=>index===6?{...row,quantity:'20'}:row));assert.equal(result.availableChipMixOz,'58');
result=calculateSampleFormulation({...standard,resinParts:'4'},rows.map((row,index)=>index===6?{...row,quantity:'20'}:row));assert.equal(result.availableChipMixOz,'57');
result=calculateSampleFormulation({...standard,totalFormulaWeightOz:'30'},rows);assert.equal(result.invalidMassBalance,true);assert.equal(result.availableChipMixOz,'');
