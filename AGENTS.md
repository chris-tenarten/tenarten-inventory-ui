...
- Test the delta and its realistic blast radius. Do not automatically rerun unrelated historical/permanent verifiers merely because they exist. Escalate the testing level if implementation crosses a higher-risk boundary, and state the level used and why in the final report.
- **Testing Level 1 — presentation-only:** focused browser/visual verification of changed UI, TypeScript, targeted ESLint, `git diff --check`, and Production build.
- **Testing Level 2 — application behavior:** focused changed-behavior tests plus directly adjacent regression coverage, browser verification, TypeScript, targeted ESLint, diff check, and Production build.
- **Testing Level 3 — high-risk/data-contract:** full relevant lifecycle/integrity/compatibility/database/browser gates for migrations, RLS/auth changes, calculation semantics, persistence contracts, issuance/snapshots, PDFs, destructive operations, or historical/Production data mutations.
